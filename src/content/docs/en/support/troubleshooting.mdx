...
During the installation of AxylOS, if you get an error like:
```
gpg: key <keyid> was created 3474 seconds in the future (time warp or clock problem)
```
it can occur when you are installing AxylOS by an ISO that has been released just now. To solve the issue, just wait 30 minutes and boot again by the ISO.
...
If AxylOS Nix installation is killed and the following error is produced:
```
...
By running ISO, at boot, if login prompt freezes, reboot and select the boot entry called **"AxylOS install medium (%ARCH%, UEFI, safe graphics, nomodeset)"**. The problem was caused by `nouveau.modeset=1` for some specific NVIDIA hardware.

#### Error: file '/vmlinuz-linux-lts' not found

If, after a system update, you get the following error at boot:
![AxylOS Snapshots Run](@assets/images/error-boot-missing-vmlinuz.png)
...
To fix it, boot in a Live Environment by AxylOS ISO, open the terminal and run the following commands:
...
Finally, in case Windows entry disappeared, land on the installed AxylOS system, open a terminal and run:
```
...
Finally, for fixing also the Linux access, burn AxylOS ISO on a USB and follow the steps explained at [Post-installation](/en/installation/install-persistent-usb/#post-installation) section.

#### Several boot entries have been disappeared

In case you are in dual boot and some entries are not shown anymore (i.e., Windows Boot Manager if you have Windows in dual boot), enter in AxylOS and run:
```shell
sudo grub-mkconfig -o /boot/grub/grub.cfg
```
Then reboot.
...
After an upgrade of your system, when you boot, after the GRUB choice, if you get only a blinking cursor, the issue could be related to the upgrade of some components that could break your system boot. To solve this issue, mount an AxylOS ISO to your computer to enter in a Live Environment, mount to `&mnt` the partition where your original system was installed and run:
```shell
sudo arch-chroot /mnt
```
...
It should be caused by an update of GDM or GNOME packages that restored the default gresource theme file. To restore the AxylOS GDM theme, run:
```shell
sudo pacman -S AxylOS-gdm-config
```
...
This issue affects both AxylOS and Kali Linux. When you run `kex --esm` and you get:
```
...
1. wine is installed
2. you are running AxylOS and Kali Linux WSL together

Uninstall wine package, close both the WSL sessions, open CMD in Windows and run `wsl --shutdown`. Run AxylOS and you should not get this error anymore.
...
wsl --shutdown
```
Then, reopen AxylOS WSL and try to run again:
```shell
kex --esm --ip -s
```
...
The reason is that the website is using the AxylOS system font instead of using its one.
...
```shell
AxylOS-theme-tweak -t <name of your favourite theme>
```
Use `AxylOS-theme-tweak -l` to get the list of themes to be chosen.

#### Restore system by snapshots in GRUB

In case you are not able to get in the system due to a broken upgrade or similar events, you can leverage on Grub BTRFS snapshots in order to restore a previous state of your system. To do this, run the system, on the GRUB select **AxylOS Snapshots**

![AxylOS Snapshots](@assets/images/athena-snapshots.png)

Choose the **snapshot** you want to restore

![AxylOS Snapshots Selection](@assets/images/athena-snapshots-selection.png)

and select the **Linux Kernel** to run

![AxylOS Snapshots Run](@assets/images/athena-snapshots-run.png)

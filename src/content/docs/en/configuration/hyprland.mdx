---
title: Hyprland
description: Hyprland environment in AxylOS.
---
...
In AxylOS, Hyprland has been been set up to make the user experience smoother in a window manager environment.
...
AxylOS's Hyprland config is based on the wonderful [dotfiles from @end-4](https://github.com/end-4/dots-hyprland).
...
To install AxylOS Hyprland environment on side of an existing environment, run:
```shell
sudo pacman -S AxylOS-hyprland-config
dconf load /org/gnome/desktop/ < /usr/share/AxylOS-hyprland-config/dconf-desktop.ini
```
Then reboot. On Login Manager, select **Hyprland** session and login.

If your theme is not correctly displayed, run `AxylOS-theme-tweak` to set or change the current environment theme.

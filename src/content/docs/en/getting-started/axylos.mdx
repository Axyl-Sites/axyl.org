---
title: Why AxylOS?
description: Motivation about the creation of AxylOS.
---
...
**AxylOS** is an open-source, **helctic-based** distribution intended to build a new concept of **desktop operating system**. Its purpose is to offer a different experience than the most used desktop distributions by providing flexibility, isolation, and only those tools that fit with the user needs and improving the access to hacking resources and learning materials.

AxylOS is designed from scratch, and, at development phase, we decided that we wanted to move away from [Linux](https://linux.org) and [Systemd](https://systemd.org) and we decided that we were going to make replacements for both of these from Scratch.

## Features
Both AxylOS souls have common aspects that bind them:
* **Open source model**: all the elements of the system are available on AxylOS [development tree](https://github.com/Axyl-Projects/axylos) for all users that wish to access or edit according to their needs.
* **Free**: AxylOS is free and accessible to everyone and it will be forever.
...
* **Flexibility**: AxylOS allows deep customization of system configurations, enabling users to tailor the environment to specific testing requirements.
* **Tailored pentesting tools**: AxylOS provides pentesting tools classified by **Cyber Roles**. Each role consists of a Cyber Security domain where the several tools are distributed on. These tools are continuously maintained in order to improve the efficiency of their usage and kept updated for getting a cleaned and secured environment.
...
* **More and more pentesting tools**: AxylOS relies on [AxylOS repository](https://github.com/AxylOS-OS/AxylOS) that contains 2800+ pentesting and security tools. These tools are continuously maintained in order to improve the efficiency of their usage and kept updated for getting a cleaned and secured environment.
* **Trusted packages**: all the packages developed on AxylOS are GPG-signed and maintained in a public and dedicated repository.
...
* **AUR (Arch User Repository)**: being based on Arch-Linux, AxylOS Arch can access to the AUR. It is a vast community-driven repository that extends the official Arch Linux repositories. It allows users to easily access and install thousands of additional packages maintained by the community. This vast collection of software expands the capabilities of AxylOS Arch and makes it easy to find and install software that may not be available in the official repositories.

### AxylOS Nix
The heritage of Nix impacts positively AxylOS according to the following characteristics:
* **Secure software**: the system is set to retrieve only secure packages. Nix Community continuously works to check vulnerable software and, if needed, set them as insecure. By Nix, AxylOS is able to inform you if a software is affected by a CVE and it prevents its installation in your system.
* **Declarative**: AxylOS uses a declarative approach to system configuration, meaning that the entire system configuration can be potentially described in a single configuration file. It actually implements an "OS as a Code". This makes it easy to understand, reproduce the system state, and to version control configurations, replicate environments, and maintain consistency across different systems, which can be valuable in pentesting scenarios.
* **Immutable System State**: The system configuration in AxylOS is treated as immutable. Instead of making changes in-place, a new configuration is generated, and the system transitions to the new state. This helps ensure consistency and reproducibility, and, mostly, ensures that the system remains in a known state, which is helpful for maintaining a secure and consistent testing environment.
...
* **Atomic Upgrades and Rollbacks**: AxylOS supports atomic upgrades and rollbacks. When a new configuration is applied, the system is atomically transitioned to the new state. If a configuration or package change introduces issues, rolling back to a known good state can be done seamlessly, preventing disruptions during pentesting activity.
...
* **Lazy Evaluation**: AxylOS employs lazy evaluation for package management. This means that packages are only built when needed, and dependencies are resolved dynamically. This helps in minimizing unnecessary builds and reducing redundancy.
